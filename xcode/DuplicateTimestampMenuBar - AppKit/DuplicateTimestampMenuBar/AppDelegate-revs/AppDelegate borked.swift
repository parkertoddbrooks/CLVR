...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    func requestPermission() {
        ...
        // Attempt to create a temporary file in each directory
        do {
            try "test".write(to: desktopURL.appendingPathComponent("clvr_test.txt"), atomically: true, encoding: .utf8)
            try fileManager.removeItem(at: desktopURL.appendingPathComponent("clvr_test.txt"))
            log("Access granted to Desktop folder")
        } catch {
            log("Access denied to Desktop folder: \(error.localizedDescription)")
        }
        
        do {
            try "test".write(to: documentsURL.appendingPathComponent("clvr_test.txt"), atomically: true, encoding: .utf8)
            try fileManager.removeItem(at: documentsURL.appendingPathComponent("clvr_test.txt"))
            log("Access granted to Documents folder")
        } catch {
            log("Access denied to Documents folder: \(error.localizedDescription)")
        }
        
        // After attempting access, verify if we have the necessary permissions
        if verifyFolderAccess() {
    ...
    func showAccessDeniedAlert() {
        ...
        failureAlert.addButton(withTitle: "Request Access Again")
