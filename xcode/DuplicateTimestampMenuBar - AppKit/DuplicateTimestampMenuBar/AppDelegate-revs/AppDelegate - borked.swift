...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    func renameFile(at path: String) {
        ...
        // Get the selected date format option
        let dateFormatOption = UserDefaults.standard.integer(forKey: "DateFormatOption")

        if name.lowercased().hasSuffix(" copy") {
            name = (name as NSString).substring(to: name.count - 5)
            if dateFormatOption == 0 {
                newName = "\(name)-copy_\(timestamp).\(fileExtension)"
            } else {
                newName = "\(name)_\(timestamp).\(fileExtension)"
            }
        } else {
        ...
        while FileManager.default.fileExists(atPath: newPath) {
            if dateFormatOption == 0 {
                newName = "\(name)-copy_\(timestamp)--\(formattedTimestamp()).\(fileExtension)"
            } else {
                newName = "\(name)_\(timestamp)--\(formattedTimestamp()).\(fileExtension)"
            }
    ...
    ///
    /// This function creates a timestamp string in the format "yyyy-MM-dd_HH-mm-ss" 
    ...
    func createSettingsWindow() -> NSWindow {
        ...
        dateFormatPopup.addItems(withTitles: [
            ...
            "name_yyyy-MM-dd-HH-mm-ss.filename-extension",
            //  Will add logic if any from ROW asks -- won't sort in order by name correectly.
            // "-",
            // "name_copy_yyyy-dd-MM-HH-mm-ss.filename-extension",
            // "name_yyyy-dd-MM-HH-mm-ss.filename-extension"
        ])

        // Bind the popup to the dateFormatOption property
        dateFormatPopup.bind(.selectedIndex, to: self, withKeyPath: "dateFormatOption", options: nil)
