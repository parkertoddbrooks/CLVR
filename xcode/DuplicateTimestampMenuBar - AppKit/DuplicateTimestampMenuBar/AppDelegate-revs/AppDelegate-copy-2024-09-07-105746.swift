...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    func animateStatusItemIcon() {
        ...
        let configuration = NSImage.SymbolConfiguration(pointSize: 17, weight: .regular)
        if let animatedImage = NSImage(systemSymbolName: "d.circle.fill", accessibilityDescription: nil)?.withSymbolConfiguration(configuration) {
            ...
            NSAnimationContext.runAnimationGroup({ context in
                ...
                NSAnimationContext.runAnimationGroup({ context in
                    ...
                    // Reset to the original icon after animation
                    button.image = originalImage
                })
            })
        } else {
            log("Failed to load system symbol: d.circle.fill", level: .error)
        }
    }
    ...
    /// Creates and configures the about window.
    func createAboutWindow() -> NSWindow {
        ...
        iconView.image = NSImage(named: "AppIcon") // Make sure you have an AppIcon in your asset catalog
