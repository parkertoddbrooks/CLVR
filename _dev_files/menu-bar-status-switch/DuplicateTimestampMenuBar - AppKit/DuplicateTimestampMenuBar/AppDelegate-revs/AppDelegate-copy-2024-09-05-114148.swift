...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Updates the state of the toggle switch in the menu.
    func updateToggleSwitch() {
        if let menu = statusItem?.menu,
           let customMenuItem = menu.items.first,
           let toggleView = customMenuItem.view as? ToggleView {
            toggleView.updateToggleState(isOn: isEnabled)
        }
    }

    /// Checks and sets the initial state of the app based on saved preferences.
...
class ToggleView: NSView {
    ...
    // This function updates the state of the toggle switch
    // Parameters:
    //   - isOn: A boolean value indicating whether the switch should be on or off
    // It sets the state of the toggleSwitch to .on if isOn is true, or .off if isOn is false
